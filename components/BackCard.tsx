...
/**
 * BackCard: Outer frame (purple/blue), inner tall orange gradient body.
 * Header (small dark top area) is solid black as requested.
 */
  ...
  return (
    ...
    >
      <div className="rounded-[30px] text-white text-[12px] flex flex-col bg-[linear-gradient(140deg,#ff8a3c_0%,#ff9f3c_35%,#ffb347_60%,#ffd26f_100%)] shadow-[0_4px_18px_-4px_rgba(255,138,60,0.45)]">
        {/* Header (was front) now solid black */}
          ...
          <div className="flex items-center gap-3 min-w-0">
            {imageUrl ? (
              <Image
                ...
                width={40}
                height={40}
                className="size-10 rounded-full object-cover border border-white/30 shadow-[0_2px_8px_rgba(0,0,0,0.5)]"
              />
            ) : null}
            {title ? (
              <p className="text-[14px] font-semibold truncate tracking-wide">
                {title}
              </p>
            ) : (
